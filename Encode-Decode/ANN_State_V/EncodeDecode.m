...
autoenc = trainAutoencoder2(orginal_data, hiddenSize, ...
    'MaxEpochs', 2, ...
    ...
    'TrainingAlgorithm', 'trainscg2');
%% trainscg, traingdm, traingda, traingdx, trainlm, trainrp, traincgf, traincgb, trainbfg, traincgp, trainoss
...
column_ranges = {[1:200], [201:450], [451:679], [680:760], [761:1059], [1060:size(orginal_data, 2)]};
...
% Define the column ranges and include the overall range
column_ranges = {
    [1:200], [201:450], [451:679], [680:760], [761:1059], [1060:size(orginal_data, 2)], 1:size(orginal_data, 2)
};
range_labels = {'1:200', '201:450', '451:679', '680:760', '761:1059', '1060:end', 'Overall'};

% Initialize arrays to store the RMSE and R^2 values for each range
rmse_values = zeros(1, length(column_ranges));
r_squared_values = zeros(1, length(column_ranges));

% Loop over each column range
for i = 1:length(column_ranges)
    % Extract the relevant columns for the current range from the original and reconstructed data
    original_range = orginal_data(:, column_ranges{i});
    reconstructed_range = reconstructed_matrix(:, column_ranges{i});
    
    % Flatten the matrices to vectors
    original_vector_flat = original_range(:);
    reconstructed_vector_flat = reconstructed_range(:);
    
    % Calculate MSE and RMSE for the current range
    mse_value = mean((original_vector_flat - reconstructed_vector_flat).^2);
    rmse_values(i) = sqrt(mse_value);
    
    % Calculate R^2 for the current range
    ss_res = sum((original_vector_flat - reconstructed_vector_flat).^2);
    ss_tot = sum((original_vector_flat - mean(original_vector_flat)).^2);
    r_squared_values(i) = 1 - (ss_res / ss_tot);
end

% Create a table with the results including range labels
results_table = table(range_labels', rmse_values', r_squared_values', ...
                      'VariableNames', {'Column_Range', 'RMSE', 'R_Squared'});
