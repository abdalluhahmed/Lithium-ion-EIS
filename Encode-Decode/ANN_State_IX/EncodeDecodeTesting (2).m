...
load 'C:\Users\msi-pc\Desktop\Li-ion paper 2\Datasets-Extracted\Testing\Input_45C02.mat'
load 'C:\Users\msi-pc\Desktop\Li-ion paper 2\Datasets-Extracted\Testing\Input_35C02.mat'
...
Input_35C02=Input_35C02(:,1:80);
Input_45C02=Input_45C02(:,1:80);

Output_35C02=Output_35C02(:,1:80);
Output_45C02=Output_45C02(:,1:80);
...
% test_vector = round(test_vector * 100) / 100;

% % Define the noise level
% noiseStd = 0.0001;
% % Generate Gaussian noise with the same size as Inputs_All
% noise = noiseStd * randn(size(test_vector));
% % Add the noise to Inputs_All
% Inputs_All_Noisy = test_vector + noise;
% 
% test_vector=Inputs_All_Noisy;


% % Assuming Inputs_All is your matrix with 120 rows
% numRows = size(test_vector, 1); % Get the number of rows
% % Generate a random permutation of row indices
% % Use the random indices to shuffle the rows
% test_vector = test_vector(randIndices, :);


% for i = 1:2:119 % Loop through the matrix in steps of 2, up to 119
%     test_vector([i, i + 1], :) = test_vector([i + 1, i], :); % Swap each pair of rows
% end
...
% Plot for 'Actual -Im(Z)/\Omega and Re(Z)/\Omega ↓'
figure;
...
hold off;
ylabel('Re(Z)/\Omega', 'FontSize', 38, 'FontWeight', 'bold');
xlabel('Frequencies', 'FontSize', 38, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold', 'Box', 'on');
grid on;
...
hold off;
ylabel('-Im(Z)/\Omega', 'FontSize', 38, 'FontWeight', 'bold');
xlabel('Frequencies', 'FontSize', 38, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold', 'Box', 'on');
grid on;
...
% Plot for 'Features'
figure;
cm = jet(size(features_test, 2));  % Store jet colormap
hold on;
for i = 1:size(features_test, 2)
    plot(features_test(:, i), 'LineWidth', 4, 'Color', cm(i, :));
end
hold off;
ylabel('Features', 'FontSize', 38, 'FontWeight', 'bold');
xlabel('Frequencies', 'FontSize', 38, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold', 'Box', 'on');
grid on;

% Plot for 'Reconstructed'
figure;
cm = jet(size(reconstructed_test_vector, 2));  % Store jet colormap
...
hold off;
ylabel('Reconstructed', 'FontSize', 38, 'FontWeight', 'bold');
xlabel('Frequencies', 'FontSize', 38, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold', 'Box', 'on');
grid on;
...
figure;
scatter(test_vector, reconstructed_test_vector, 'Marker', 'o', 'MarkerFaceColor', 'blue', 'MarkerEdgeColor', 'blue', 'SizeData', 100); % Adjust SizeData for marker size
hold on;
p = polyfit(test_vector, reconstructed_test_vector, 1); % Perform linear regression
plot(test_vector, polyval(p, test_vector), 'r-', 'LineWidth', 4); % Set line width to 4 for the fit line
hold off;
xlabel('-Im(Z)/\Omega and Re(Z)/\Omega ↓', 'FontSize', 40, 'FontWeight', 'bold');
ylabel('Reconstructed', 'FontSize', 40, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold', 'Box', 'on'); % Set axes tick label properties and enable box
grid on;
axis square;
...
text(0.1, 0.9, ['R^2 = ', num2str(R_squared)], 'Units', 'normalized', 'FontSize', 36, 'FontWeight', 'bold'); % Set R-squared text size to 40 and bold
