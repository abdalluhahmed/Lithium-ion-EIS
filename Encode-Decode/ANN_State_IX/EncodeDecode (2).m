...
load 'C:\Users\msi-pc\Desktop\Li-ion paper 2\Datasets-Extracted\Training\Inputs_All'
...
A=Inputs_All(:,1:80);
B=Inputs_All(:,201:280);
C=Inputs_All(:,451:530);
D=Inputs_All(:,680:759);
E=Inputs_All(:,761:840);
F=Inputs_All(:,1060:1139);

AA=Output_All(:,1:80);
BB=Output_All(:,201:280);
CC=Output_All(:,451:530);
DD=Output_All(:,680:759);
EE=Output_All(:,761:840);
FF=Output_All(:,1060:1139);

Inputs_All=[A,B,C,D,E,F];
Output_All=[AA,BB,CC,DD,EE,FF];




 % Inputs_All = round(Inputs_All * 100) / 100;


% % Define the noise level
% noiseStd = 0.0001;
% % Generate Gaussian noise with the same size as Inputs_All
% noise = noiseStd * randn(size(Inputs_All));
% % Add the noise to Inputs_All
% Inputs_All_Noisy = Inputs_All + noise;
% 
% Inputs_All=Inputs_All_Noisy;
% If you need to round the noisy data to two decimal places as before

% Now Impedance_Magnitude contains the impedance magnitude for each corresponding pair of real and imaginary components.

% Inputs_All = round(Inputs_All * 10) / 10;

% Input_35C02_75=Input_35C02(:,1:end);


% Assuming Inputs_All is your matrix with 120 rows
% numRows = size(Inputs_All, 1); % Get the number of rows
% Generate a random permutation of row indices
% randIndices = randperm(numRows);
% Use the random indices to shuffle the rows
% Inputs_All = Inputs_All(randIndices, :);
...
% Inputs_All = [Inputs_All];
% 
% for i = 1:2:119 % Loop through the matrix in steps of 2, up to 119
%     Inputs_All([i, i + 1], :) = Inputs_All([i + 1, i], :); % Swap each pair of rows
% end
...
for i = 1:20
    ...
    scatter(i * ones(size(encoder_weights, 1), 1), encoder_weights(:, i), 'filled', ...
        'CData', cmap(colorIdx, :), 'LineWidth', 4);
end

hold off;
title('Encoder Weights Comparison', 'FontSize', 40, 'FontWeight', 'bold');
xlabel('Neurons index', 'FontSize', 40, 'FontWeight', 'bold');
ylabel('Encoder weights', 'FontSize', 40, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold'); % Set axes tick label properties
grid on;
box on;
...
for i = 1:120
    ...
    scatter(i * ones(size(decoder_weights, 2), 1), decoder_weights(i, :), 'filled', ...
        'CData', cmap(colorIdx, :), 'LineWidth', 4);
end

hold off;
title('Decoder Weights Comparison', 'FontSize', 40, 'FontWeight', 'bold');
xlabel('Neurons index', 'FontSize', 40, 'FontWeight', 'bold');
ylabel('Decoder weights', 'FontSize', 40, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold'); % Set axes tick label properties
grid on;
box on;
set(gcf, 'Color', 'w'); % Set background color to white
...
figure;
scatter(orginal_data(:, i), reconstructed_matrix(:, i), 'Marker', 'o', 'MarkerFaceColor', 'blue', 'MarkerEdgeColor', 'blue', 'SizeData', 100); % Adjust SizeData for marker size
hold on;
...
plot(x_fit, y_fit, 'r-', 'LineWidth', 4); % Set line width to 4
...
text(min(orginal_data(:, i)), max(reconstructed_matrix(:, i)), ['R^2 = ' num2str(r_squared)], 'FontSize', 36, 'FontWeight', 'bold'); % Set R-squared text size to 40 and bold

hold off;

xlabel('-Im(Z)/\Omega and Re(Z)/\Omega ↓', 'FontSize', 40, 'FontWeight', 'bold', 'Interpreter', 'tex');
ylabel('Reconstructed', 'FontSize', 40, 'FontWeight', 'bold');
set(gca, 'FontSize', 40, 'FontWeight', 'bold'); % Set axes tick label properties
grid on;
box on;
axis square;
